...
<template>
  <main class="flex min-h-[calc(100vh-65px)] items-center justify-center px-6 py-12">
    <form class="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/60" @submit.prevent="requestReset">
      ...
      <div class="mt-8">
        <label class="block text-sm font-medium">Email<input v-model="email" :disabled="loading" type="email" autocomplete="email" required autofocus class="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 outline-none disabled:opacity-60 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100" /></label>
      </div>
      <AsyncButton type="submit" :loading="loading" :disabled="!formValid" loading-text="Mengirim link…" class="mt-6 w-full rounded-xl bg-slate-900 px-4 py-3 font-semibold text-white hover:bg-emerald-600">Kirim link reset</AsyncButton>
