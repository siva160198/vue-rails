...
const cards = computed(() => dashboard.value ? [
  ...
  { label: 'Active sessions', value: dashboard.value.metrics.active_sessions, change: 'Live', trend: 'up', icon: Activity },
  { label: 'Database', value: 'PostgreSQL', change: 'Connected', trend: 'up', icon: Database },
  { label: 'Authorization', value: 'Pundit', change: 'Protected', trend: 'up', icon: ShieldCheck },
] : [])
      ...
      <template v-else>
        ...
        <div class="mt-6 grid gap-6 xl:grid-cols-3">
          <section class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03] xl:col-span-2">
            <div class="flex items-center justify-between"><div><h2 class="text-lg font-semibold text-gray-900 dark:text-white">Workspace overview</h2><p class="mt-1 text-sm text-gray-500">System activity for this application</p></div><select class="rounded-lg border border-gray-200 bg-transparent px-3 py-2 text-sm text-gray-500 dark:border-gray-800"><option>Last 7 days</option></select></div>
