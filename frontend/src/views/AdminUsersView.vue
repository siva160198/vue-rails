    ...
    <div class="mx-auto max-w-[1536px]">
      ...
      <div class="overflow-x-auto rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <table class="w-full text-left text-sm"><thead class="bg-gray-50 text-xs uppercase text-gray-500 dark:bg-white/[0.02]"><tr><th class="px-5 py-3">Email</th><th class="px-5 py-3">Role</th><th class="px-5 py-3">Status</th><th class="px-5 py-3">Verified</th><th class="px-5 py-3">Action</th></tr></thead>
          <tbody><tr v-for="user in users" :key="user.id" class="border-t border-gray-100 dark:border-gray-800"><td class="px-5 py-4 font-medium dark:text-white">{{ user.email_address }}<span v-if="user.id === admin?.id" class="ml-2 text-xs text-brand-500">Anda</span></td><td class="px-5 py-4"><select v-model="user.role" :disabled="user.id === admin?.id" class="rounded-lg border border-gray-200 bg-transparent px-3 py-2 dark:border-gray-700 dark:text-white"><option v-for="role in roles" :key="role.key" :value="role.key">{{ role.name }}</option></select></td><td class="px-5 py-4"><label class="inline-flex items-center gap-3" :class="user.id === admin?.id ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'"><input v-model="user.active" type="checkbox" class="peer sr-only" :disabled="user.id === admin?.id" :aria-label="`Status ${user.email_address}`" /><span class="relative h-6 w-11 rounded-full bg-gray-300 transition-colors after:absolute after:left-0.5 after:top-0.5 after:h-5 after:w-5 after:rounded-full after:bg-white after:shadow-sm after:transition-transform peer-checked:bg-success-500 peer-checked:after:translate-x-5 peer-focus-visible:ring-4 peer-focus-visible:ring-brand-100 dark:bg-gray-700 dark:peer-checked:bg-success-600"></span><span :class="user.active ? 'text-success-700 dark:text-success-500' : 'text-gray-500'" class="min-w-14 text-sm font-medium">{{ user.active ? 'Aktif' : 'Nonaktif' }}</span></label></td><td class="px-5 py-4 text-gray-500">{{ user.email_verified_at ? 'Ya' : 'Belum' }}</td><td class="px-5 py-4"><button :disabled="user.id === admin?.id" class="rounded-lg bg-gray-900 px-4 py-2 text-xs font-semibold text-white disabled:opacity-40" @click="saveUser(user)">Simpan</button></td></tr></tbody>
        </table>
