...
<template><DataTable :items="items" :columns="columns" :loading="loading" :empty-text="t('security.empty_history')" server-mode cursor-mode :total="pagination.total" :next-cursor="pagination.next_cursor || ''" :previous-cursor="pagination.previous_cursor || ''" :has-next="pagination.has_next" :has-previous="pagination.has_previous" @request="load"><template #cell-created_at="{ item }"><span class="text-gray-500">{{ new Date(item.created_at).toLocaleString(locale) }}</span></template><template #cell-ip_address="{ item }"><span class="text-gray-500">{{ item.ip_address || '—' }}</span></template><template #cell-user_agent="{ item }"><span class="line-clamp-2 text-gray-500" :title="item.user_agent">{{ item.user_agent || t('sessions.unknown_device') }}</span></template></DataTable></template>
