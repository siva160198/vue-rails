...
async function startTotpEnrollment(stepUpToken) {
  action.value = "totp";
  try { totpSetup.value = await apiFetch("/api/v1/account_security/request_totp", { method: "POST", headers: { "X-Step-Up-Token": stepUpToken } }); }
  catch (error) { toast.error(error.message); } finally { action.value = ""; }
}
async function submitTotp() {
  if (action.value) return;
  const field = "code";
  const valid = await formErrors.validate({ code: () => /^\d{6}$/.test(totpForm.value.code) ? "" : t("validation.otp") }, formElement.value);
  if (!valid) return;
  action.value = "totp";
  try {
    await apiFetch("/api/v1/account_security/verify_totp", { method: "POST", body: JSON.stringify({ code: totpForm.value.code }) }); totpEnabled.value = true; totpSetup.value = null; toast.success(t("security.totp_enabled")); emit("saved");
...
<template>
  ...
  <div v-else-if="feature === 'totp'" class="space-y-4"><StepUpPrompt v-if="totpEnabled" purpose="totp_disable" @verified="disableTotp" @cancel="emit('saved')" /><StepUpPrompt v-else-if="!totpSetup" purpose="totp_enroll" @verified="startTotpEnrollment" @cancel="emit('saved')" /><form v-else ref="formElement" class="space-y-4" @submit.prevent="submitTotp"><div class="rounded-xl bg-gray-50 p-4 text-sm dark:bg-gray-800"><p>{{ t('security.totp_setup_hint') }}</p><code class="mt-2 block break-all font-semibold">{{ totpSetup.secret }}</code></div><FormField :label="t('auth.otp')" :error="formErrors.errorFor('code')"><TextInput v-model="totpForm.code" name="code" inputmode="numeric" maxlength="6" /></FormField><AsyncButton type="submit" :loading="action === 'totp'" :disabled="Boolean(action)" class="rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-semibold text-white">{{ t('common.verify') }}</AsyncButton></form></div>
